...
console.log('[GAMX Firefox Panel] Connecting to background script...');
const port = browser.runtime.connect({ name: "devtools-panel" });
console.log('[GAMX Firefox Panel] Port created:', port);
...
function clearAllDataLocal() {
  try {
    ...
    console.log('[GAMX Firefox] clearAllDataLocal completed successfully');
  } catch (e) {
    console.warn('[GAMX Firefox] Local clear failed:', e);
  }
}
...
if (reloadExtensionBtn) {
  reloadExtensionBtn.addEventListener('click', (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        // browser.runtime.reload will invalidate context; the panel will disconnect
...
port.onMessage.addListener((msg) => {
  console.log('[GAMX Firefox Panel] Message received:', msg.type);
  try {
    if (msg.type === "CONNECTION_READY") {
      console.log('[GAMX Firefox Panel] Connection ready!');
      ...
      console.log('[GAMX Firefox Panel] API_CALLS_DATA received, count:', msg.data?.length);
...
browser.runtime.onMessage.addListener((msg) => {
  console.log('[GAMX Firefox Panel] Runtime message received:', msg.type);
  try {
    if (msg.type === "API_CALL_DETECTED") {
      console.log('[GAMX Firefox Panel] API_CALL_DETECTED:', msg.endpoint);
...
if (clearBtn) {
  clearBtn.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('[GAMX Firefox] Clear button clicked');
    
    try {
      // Removed confirm dialog to avoid blocking issues in DevTools
      console.log('[GAMX Firefox] Clearing data...');
      ...
      try {
        ...
        console.log('[GAMX Firefox] Clear data message sent successfully');
      } catch (err) {
        console.error('[GAMX Firefox] Failed to send CLEAR_DATA:', err);
        // showStatus('Data cleared locally (background notification failed)', 'warning');
      }
    } catch (err) {
      console.error('[GAMX Firefox] Error in clear handler:', err);
      showStatus('Error clearing data: ' + err.message, 'error');
    }
  });
} else {
  console.error('[GAMX Firefox] clearBtn element not found!');
}
...
async function loadSecurityRules() {
  try {
    ...
    const stored = await browser.storage.local.get('securityRulesConfig');
...
async function saveSecurityRules() {
  try {
    await browser.storage.local.set({ securityRulesConfig });
...
async function resetSecurityRules() {
  ...
  try {
    await browser.storage.local.remove('securityRulesConfig');
